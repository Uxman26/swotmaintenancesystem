...
Vue.component('calendar-day', {
    ...
    methods: {
        ...
        eventTimeSpan(user_event) {
            var startDate   = moment(user_event.woffice_event_date_start).format('dddd,Do YYYY, h:mm A');
            var endDate     = moment(user_event.woffice_event_date_end).format('dddd,Do YYYY, h:mm A');
            ...
            if (start === end && sameDate) {
                return this.exchanger.full_day + ' - ' + visibility;
            }

            if (!sameDate) {
                return `${startDate} - ${endDate} <div> ${visibility}</div>`;
            }
